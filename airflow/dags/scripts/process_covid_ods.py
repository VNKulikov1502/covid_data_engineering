...
def main():
    ...
    # Извлекаем дату из имени файла (source_file), так надежнее всего
    ...
    # --- 3. Нормализация стран (Data Cleaning) ---
    # Приводим "грязные" имена к стандарту ДО агрегации
    ...
    # --- 4. Агрегация ---
    # Группируем уже по нормализованному имени
    ...
    # --- 5. Запись (Dynamic Partition Overwrite) ---
    ...
    # Логика: Если таблицы нет - создать, если есть - перезаписать партицию
    try:
        spark.table(target_table) # Проверка существования
        # ВАЖНО: overwritePartitions автоматически заменит данные ТОЛЬКО за report_date
