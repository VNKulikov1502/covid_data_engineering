...
def main():
    ...
    # ==========================================
    # ШАГ 1: Создаем Измерение (DIMENSION) - dim_location
    # Это SCD-0 / Историческое измерение, включает данные за все годы
    # ==========================================

    # 1. Читаем RAW Population (ВСЕ ГОДЫ)
    pop_raw = spark.table("iceberg.raw.country_population")

    # 2. Формируем таблицу измерения
    # Ключ теперь зависит от СТРАНЫ + ГОДА, чтобы население было актуально для каждого года отчета
    dim_location = pop_raw.select(
        # Генерируем уникальный ключ: SHA2(Название Страны + Год)
    ...
    # 3. Записываем DIM (Перезаписываем целиком)
    ...
    # ==========================================
    # ШАГ 2: Создаем Факты (FACT) - fact_covid
    # ==========================================

    # 1. Читаем ODS за конкретную дату
    ...
    # 2. Добавляем колонку "Год отчета" для джойна с dim_location
    ods_enriched = ods_df.withColumn("report_year", year(col("report_date")))

    # 3. Читаем измерение
    dim_df = spark.table("iceberg.dds.dim_location")

    # 4. ДЖОЙН ПО ДВУМ КЛЮЧАМ (СТРАНА + ГОД)
    # Соединяем агрегированные данные ODS с актуальным населением из DIM
    fact_df = ods_enriched.join(
        ...
        col("report_date"),
        col("location_key"), # Ссылка на измерение (ID страны+года)
        col("confirmed"),
        col("deaths"),
        col("recovered"),
        col("active"),
    ...
    # 5. Проверка качества (необязательно)
    ...
    # 6. Запись FACT (Dynamic Partition Overwrite)
