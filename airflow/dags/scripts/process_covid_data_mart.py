...
def main():
    ...
    # 2. Удаляем таблицу (один раз), если она была создана неправильно ранее.
    # В ПРОДАКШЕНЕ ЭТУ СТРОКУ НУЖНО БУДЕТ УБРАТЬ ПОСЛЕ ПЕРВОГО УСПЕШНОГО ЗАПУСКА,
    # но сейчас она нужна, чтобы починить структуру таблицы.
    # spark.sql("DROP TABLE IF EXISTS iceberg.data_mart.covid_analytics PURGE")

    # 3. DDL Таблицы (Гарантируем Partitioning)
    ddl = """
